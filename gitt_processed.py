...
for i in range(len(tpulse_discharge)):
    ...
    if fit_Rp:
        ...
        get_voltage, t_pulse_data_discharge, voltage_data, p0=[tau_D_est, np.mean(R_est, R_est_1)], bounds=(np.array([0,min(R_est, R_est_1)]), np.array([np.inf, max(R_est, R_est_1)]))
        )
    else:
        popt_p, _ = scipy.optimize.curve_fit(
